...
// ── tiny step row (used inside the progress card) ─────────────────────────────
...
export default function Home() {
    ...
    pollRef.current = setInterval(async () => {
      try {
        ...
        if (data.status === "ready") {
          ...
          setError("Indexing failed. You can still click Index manually from the library.");
          stopPolling();
          setDocuments((prev) =>
  ...
  const uploadFile = async (file: File) => {
    ...
    try {
      ...
      // Add to library immediately (shown with "indexing" badge)
      setDocuments((prev) => [data, ...prev]);

      // Activate progress card
      setActiveUpload({
        id: data.id,
  ...
  return (
    <main className="relative min-h-screen overflow-hidden text-white">
      ...
      <div className="pointer-events-none fixed inset-0 -z-10">
        ...
        <div className="absolute top-0 inset-x-0 h-32 bg-gradient-to-b from-[#0a0f1e] to-transparent" />
      </div>
      ...
      <div className="relative max-w-5xl mx-auto px-6 py-16">
        ...
        <div className="text-center mb-14">
          <div className="inline-flex items-center gap-2 px-3 py-1 mb-10 bg-white/[0.04] border border-white/[0.08] rounded-full text-xs font-medium text-slate-400 backdrop-blur-sm">
          ...
          <p className="text-sm text-slate-500 mt-2 font-light">Your second brain for documents</p>
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-10">
          {[
            ...
            <div key={f.label}
              className="px-3.5 py-1.5 bg-white/[0.03] border border-white/[0.06] rounded-full text-xs text-slate-400 backdrop-blur-sm">
        ...
        {/* ──── Converter Banner ──── */}
        <div className="mb-10">
          <div className="relative overflow-hidden rounded-2xl p-5 bg-gradient-to-r from-violet-500/[0.09] via-fuchsia-500/[0.05] to-cyan-500/[0.09] border border-violet-400/25 backdrop-blur-sm shadow-lg shadow-violet-900/20">
            ...
            <div className="flex items-center justify-between gap-6 flex-wrap">
              <div className="flex items-center gap-4">
                <div className="flex-shrink-0 w-11 h-11 bg-gradient-to-br from-violet-500/30 to-cyan-500/30 border border-violet-400/30 rounded-xl flex items-center justify-center">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">Upload .docx → auto-convert to PDF → download or start chatting</p>
                </div>
              </div>
              <a href="/converter"
        ...
        <div
          ...
          onDrop={handleDrop}
          className={`relative rounded-2xl p-14 text-center transition-all backdrop-blur-sm
            ${dragActive
        ...
        >
          ...
          <div className="pointer-events-none relative">
            <div className="inline-flex items-center justify-center w-16 h-16 mb-5 bg-gradient-to-br from-violet-500/20 to-cyan-500/20 border border-white/10 rounded-2xl">
              {uploading ? (
            ...
            <p className="text-lg font-medium mb-1.5 text-slate-100">
              {uploading ? "Uploading…" : dragActive ? "Release to upload" : "Drop a PDF or click to browse"}
            </p>
        ...
        {activeUpload && uploadPhase && (
          <div className={`mt-5 rounded-2xl backdrop-blur-sm overflow-hidden transition-all
            ${uploadPhase === "ready"
              ? "border border-emerald-500/30 bg-emerald-500/[0.04] shadow-lg shadow-emerald-900/20"
              : uploadPhase === "failed"
            ...
            {/* file info row */}
            <div className="flex items-center gap-3.5 px-5 py-4 border-b border-white/[0.05]">
            ...
            {uploadPhase === "ready" && (
              <div className="px-5 pb-5">
                <p className="text-sm text-slate-300 mb-3">
                  Your document is ready. You can start chatting now.
                </p>
                <a
                  href={`/chat?doc=${activeUpload.id}`}
                  className="flex items-center justify-center gap-2 w-full px-4 py-3 bg-gradient-to-r from-violet-600 to-cyan-600 hover:from-violet-500 hover:to-cyan-500 border border-violet-500/40 rounded-xl text-sm font-semibold transition-all shadow-lg shadow-violet-700/25"
                >
        ...
        {documents.length > 0 && (
          <div className="grid grid-cols-2 gap-3 mt-8">
            <div className="px-5 py-4 bg-white/[0.02] border border-white/[0.07] rounded-xl backdrop-blur-sm">
        ...
        <div className="mt-12">
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-xl font-semibold text-slate-100">Your Library</h2>
          ...
          {documents.length === 0 ? (
            <div className="text-center py-14 bg-white/[0.02] border border-white/[0.05] rounded-2xl backdrop-blur-sm">
            ...
            <div className="space-y-2">
              {documents.map((doc) => (
                <div key={doc.id}
                  className="group p-4 bg-white/[0.02] border border-white/[0.07] rounded-xl hover:bg-white/[0.04] hover:border-white/[0.12] transition-all backdrop-blur-sm">
                  <div className="flex items-center justify-between gap-4 flex-wrap">
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      {/* ready: full action row */}
                      {doc.status === "ready" && (
                        <>
                          <a href={`/mindmap?doc=${doc.id}`}
                            className="px-2.5 py-1 text-[11px] font-medium text-slate-400 hover:text-slate-200 border border-white/[0.08] hover:border-white/[0.18] hover:bg-white/[0.05] rounded-md transition-all">
                            Map
                          </a>
                          ...
                          <a href={`/chat?doc=${doc.id}`}
                            className="px-3 py-1 text-[11px] font-semibold bg-gradient-to-r from-violet-600 to-cyan-600 hover:from-violet-500 hover:to-cyan-500 border border-violet-500/40 rounded-md transition-all shadow-md shadow-violet-600/20">
                            Chat →
                          </a>
                        </>
                      )}
                      ...
                      {(doc.status === "pending" || doc.status === "failed" || doc.status === "index_failed") && (
                        <button onClick={() => reprocessDocument(doc.id)}
                          className="px-3 py-1 text-[11px] font-semibold bg-gradient-to-r from-violet-600 to-fuchsia-600 hover:from-violet-500 hover:to-fuchsia-500 border border-violet-500/40 rounded-md transition-all">
                          Reprocess
                        </button>
                      )}
                      ...
                      <span className={`px-2 py-0.5 text-[10px] rounded-full font-medium uppercase tracking-wide ${
                        ...
                        {doc.status === "ready" ? "ready"
                          ...
                          : doc.status === "index_failed" ? "failed"
                          : doc.status}
                      </span>
                    </div>
                  </div>
        ...
        <footer className="mt-20 pt-6 border-t border-white/[0.05] text-center text-[11px] text-slate-600">
          ...
          <div className="w-full max-w-sm bg-[#0d1224] border border-white/[0.1] rounded-2xl overflow-hidden shadow-2xl shadow-black/60">
            <div className="p-6">
              ...
              <p className="text-xs text-slate-500 leading-relaxed mb-5">
                This permanently deletes the PDF, all chat history, and its vector embeddings from the knowledge base. This cannot be undone.
              </p>
